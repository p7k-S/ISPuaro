...
### Желательно
- NVD API Key (без него скрипт работает, но есть ограничение по запросам)
## 🚀 Быстрый старт  
```bash
git clone https://github.com/p7k-S/ISPuaro/
cd ISPuaro/scripts/
python info.py                 # базовый анализ всех бинарников -> ISPuaro/info/
python gen_sbom.py --api-key ваш_ключ --dir путь_к_директории --output выходной_файл.json  # параметры не обязательны
